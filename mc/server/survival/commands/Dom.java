...
implements CommandExecutor, TabCompleter
{
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args)
	{
		if (sender instanceof Player)
		{
			...
			if (rank.equalsIgnoreCase("administrator") & !Commands.getInstance().getReader("dom").isForAdministrators() ||
					rank.equalsIgnoreCase("moderator") & !Commands.getInstance().getReader("dom").isForModerators() ||
					rank.equalsIgnoreCase("gracz") & !Commands.getInstance().getReader("dom").isForPlayers())
			{
			...
			if (args.length == 0)
				TaskLib.getInstance().runSyncLater(() -> InventoryLib.createNewInventory(player, 27, ChatColor.translateAlternateColorCodes('&', "&c&lDOMKI"), "domki"),10);
			else
