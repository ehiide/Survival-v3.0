...
implements CommandExecutor, TabCompleter
{
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            if (rank.equalsIgnoreCase("administrator") & !Commands.getInstance().getReader("invsee").isForAdministrators() ||
                    rank.equalsIgnoreCase("moderator") & !Commands.getInstance().getReader("invsee").isForModerators() ||
                    rank.equalsIgnoreCase("gracz") & !Commands.getInstance().getReader("invsee").isForPlayers())
            {
            ...
            if (args.length == 0)
                ChatManager.sendNotification(player, "Mam Ci wyswietlic Twoj ekwipunek? Sobie kurwa sluzacego znalazl! Kliknij E...", ChatManager.NotificationType.ERROR);
            else
            {
                ...
                for (Player dplayer : Bukkit.getOnlinePlayers())
                    ...
                    {
                        player.openInventory(dplayer.getInventory());
                        return true;
                    }
