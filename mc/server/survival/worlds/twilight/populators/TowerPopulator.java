...
{
    private static final boolean shouldBePopulated = (boolean) FileManager.getInstance().getConfigValue("function.twilight.structures.twilight-tower");
    ...
    public static void populate(World world, Random random, Chunk chunk, final int y)
    {
        ...
        if (material.isSolid() && material == Material.GRASS_BLOCK)
            if (MathLib.chanceOf(0.05))
                Twilight.getInstance().getTwilightGenerator().generate(world, finalX, y - 1, finalZ, Twilight.StructureType.TWILIGHT_TOWER);
    }
}