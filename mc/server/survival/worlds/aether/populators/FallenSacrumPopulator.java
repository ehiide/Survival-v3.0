...
public class FallenSacrumPopulator
{
    private static final boolean shouldBePopulated = (boolean) FileManager.getInstance().getConfigValue("function.aether.structures.fallen-sacrum");
    ...
    public static void populate(final World world, final Random random, final Chunk chunk, final int y)
    {
        ...
        if (material.isSolid() && material == Material.TUFF)
            if (MathLib.chanceOf(60))
                Aether.getInstance().getAetherGenerator().generate(world, finalX, y, finalZ, Aether.StructureType.AETHER_SACRUM);
    }
}