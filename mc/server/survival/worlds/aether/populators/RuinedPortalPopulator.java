...
{
    private static final boolean shouldBePopulated = (boolean) FileManager.getInstance().getConfigValue("function.aether.structures.ruined-portal");
    ...
    public static void populate(final World world, final Random random, final Chunk chunk, final int y)
    {
        ...
        if (material.isSolid() && material == Material.GRASS_BLOCK)
            if (MathLib.chanceOf(5))
                Aether.getInstance().getAetherGenerator().generate(world, finalX, y, finalZ, Aether.StructureType.AETHER_RUINED_PORTAL);
    }
}